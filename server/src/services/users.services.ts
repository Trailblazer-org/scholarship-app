...
const UsersServices = {
   ...
   //  },
    ...
    registerUser: async (newUser: IUser) => {
      try {
          ...
          const dataUser: IUser = {
              ...
              major: newUser.major,
              funding_need: newUser.funding_need,
              preference: newUser.preference,
              created_at: new Date(),
              updated_at: new Date(), 
          };

          const createdUser = await UsersRepository.createUser(dataUser);
          return createdUser;
      } catch (error) {
          console.log("Error registering user in service", error);
